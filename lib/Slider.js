...
var min=new _bignumber2.default(_this.props.minimumValue);
var max=new _bignumber2.default(_this.props.maximumValue);
var diff=max.minus(min);
var value=diff.multipliedBy(ratio);

if(_this.props.step){
var stepped=value.dividedBy(_this.props.step);
var rounded=new _bignumber2.default(stepped.toFixed(0)).multipliedBy(
_this.props.step);
...
_this.props.minimumValue+rounded.toNumber()));
...
return Math.max(
_this.props.minimumValue,
Math.min(_this.props.maximumValue,value.plus(min).toNumber()));

},_this.

_getCurrentValue=function(){return _this.state.value.__getValue();},_this.
...
pointerEvents:'none',__source:{fileName:_jsxFileName,lineNumber:555}}));
