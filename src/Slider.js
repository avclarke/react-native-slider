...
export default class Slider extends PureComponent {
  ...
  _getValue = (gestureState: Object) => {
    ...
    const min = new BigNumber(this.props.minimumValue);
    const max = new BigNumber(this.props.maximumValue);
    const diff = max.minus(min);
    const value = diff.multipliedBy(ratio);

    if (this.props.step) {
      const stepped = value.dividedBy(this.props.step);
      const rounded = new BigNumber(stepped.toFixed(0)).multipliedBy(
        this.props.step
      );
      ...
      return Math.max(
        ...
        Math.min(
          ...
          this.props.minimumValue + rounded.toNumber()
        )
      );
    }
    ...
    return Math.max(
      this.props.minimumValue,
      Math.min(this.props.maximumValue, value.plus(min).toNumber())
    );
  };
